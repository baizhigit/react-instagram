...
export const useFollowButtonStyles = makeStyles({
  button: {
    ...
    marginTop: ({ side }: { side: boolean }) => `${side ? '0px !important' : '10px !important'}`,
  },
});
...
export const useUserCardStyles = makeStyles({
  avatar: {
    width: ({ avatarSize = 44 }: { avatarSize: number }) => avatarSize,
    height: ({ avatarSize = 44 }: { avatarSize: number }) => avatarSize,
  },
  typography: {
...
export const useProfilePictureStyles = makeStyles({
  person: {
    color: '#ffffff',
    height: ({ size = 150 }: { size: number }) => size,
    width: ({ size = 150 }: { size: number }) => size,
  },
  wrapper: {
    background: '#DBDBDB',
    width: ({ size = 150 }: { size: number }) => size,
    ...
    placeItems: 'center',
    // '&:hover': {
    //   cursor: ({ isOwner }: { isOwner: boolean }) => (isOwner ? 'pointer' : 'default')
    // },
  },
  section: {
    display: 'grid',
...
// EditProfilePage component: /components/pages/EditProfile.js
